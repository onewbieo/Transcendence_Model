...
## 19) Protected Data Origin Trace Plan
Goal: identify where blocked secret values came from and prevent repeat incidents.

### 19.1 Trace which commit introduced secret-bearing file
- Run:
  - `git log --all --name-status -- ft_transcendence/backend/env.docker`
  - `git log --all --name-status -- ft_transcendence/backend/.env.docker`
- If GitHub reported a commit id, inspect it directly:
  - `git show <commit_id> -- ft_transcendence/backend/env.docker`
- Pass criteria:
  - first introducing commit id is identified and documented.

### 19.2 Identify who/when
- Run:
...
- Record:
  ...
  - branch/context
- Pass criteria:
  - team knows exact origin of the leaked value.

### 19.3 Verify whether value was generated by app or copied from config
- Check codebase for generation logic:
  - `rg -n "GOOGLE_CLIENT_ID|GOOGLE_CLIENT_SECRET|GOCSPX|apps.googleusercontent.com" .`
- Check if any script writes env files:
  - `rg -n "env.docker|\\.env\\.docker|writeFile|cat >|cp .*env" .`
- Expected result for this repo:
  - OAuth values are configuration inputs, not auto-generated by app code.
- Pass criteria:
  - confirmed as manually provided credentials, not runtime-generated data.

### 19.4 Remediation and prevention
- Keep real secrets only in local:
  - `ft_transcendence/backend/.env`
  - `ft_transcendence/backend/.env.docker`
- Keep only placeholders in tracked files:
  - `ft_transcendence/backend/.env.example`
  - `ft_transcendence/frontend/.env.example`
- Ensure ignored:
  - `.env`
  - `.env.*` (except `*.example`)
